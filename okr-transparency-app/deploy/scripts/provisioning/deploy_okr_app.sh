...
PROJECT_ID="gen-lang-client-0913302758"
REGION="us-west1"
...
WORKDIR="${HOME}/okr-cloud-run-deploy-${TAG}-$(date +%Y%m%d%H%M%S)"
...
gcloud services enable run.googleapis.com artifactregistry.googleapis.com secretmanager.googleapis.com firestore.googleapis.com datastore.googleapis.com iap.googleapis.com cloudbuild.googleapis.com --project "$PROJECT_ID" --quiet

log "Ensure Firestore database"
if gcloud firestore databases describe --database="(default)" --project "$PROJECT_ID" >/dev/null 2>&1; then
  echo "Firestore default database already exists."
else
  echo "Creating Firestore default database in nam5."
...
if gcloud artifacts repositories describe "$AR_REPO" --location="$REGION" --project "$PROJECT_ID" >/dev/null 2>&1; then
  ...
  gcloud artifacts repositories create "$AR_REPO" --repository-format=docker --location="$REGION" --description="Internal Docker images for UnitX tools" --project "$PROJECT_ID" --quiet
fi

log "Ensure runtime service account and IAM"
...
gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:${CLOUDBUILD_SA}" --role="roles/artifactregistry.writer" --condition=None --quiet >/dev/null || true
...
log "Build and push image with Cloud Build"
...
log "Ensure IAP OAuth brand"
if gcloud iap oauth-brands list --project "$PROJECT_ID" --format='value(name)' --limit=1 | grep -q .; then
  echo "IAP OAuth brand already exists."
else
  gcloud iap oauth-brands create --application_title="OKR Transparency App" --support_email="$ACCOUNT" --project "$PROJECT_ID" --quiet || true
fi

log "Enable IAP on Cloud Run"
gcloud run services update "$SERVICE" --region "$REGION" --project "$PROJECT_ID" --iap --quiet

log "Grant IAP access"
